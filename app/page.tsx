  ...
  return (
    <div className="bg-[#f7f9fb] font-[family-name:var(--font-inter)] text-[#191c1e] overflow-x-hidden min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        {/* Hero Section — mobile-first, desktop two-col */}
        <section className="relative overflow-hidden bg-gradient-to-br from-white to-[#c9e6ff]/20 pt-0">
          <div className="absolute -top-20 -right-20 w-80 h-80 bg-[#006591]/10 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute -bottom-20 -left-20 w-96 h-96 bg-[#006e2f]/10 rounded-full blur-3xl pointer-events-none" />

          {/* Mobile hero: image + badge (only on <lg) */}
          <div className="lg:hidden relative w-full px-4 pt-2">
            <div className="relative w-full overflow-hidden rounded-3xl shadow-lg">
              <img
                alt="Petualangan AR"
                className="w-full object-cover object-top"
                style={{ height: 320, animation: 'float 4s ease-in-out infinite' }}
              ...
              />
              {/* Badge overlay */}
              <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-sm px-3 py-2 rounded-2xl shadow-xl z-20"
                style={{ animation: 'bounce 3s infinite' }}>
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-[#785a00] text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>stars</span>
                  <div>
                    <p className="text-[10px] text-[#3e4850] leading-none mb-0.5">Lencana Baru</p>
                    <p className="text-[13px] font-bold leading-none">Penjaga Sungai</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-[1200px] mx-auto px-5 pt-5 pb-7 lg:py-16 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">

            {/* Left: Text + CTA */}
            <div className="lg:col-span-6 space-y-4">
              <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[#6bff8f] text-[#007432] text-xs font-semibold">
                <span className="material-symbols-outlined text-sm">explore</span> Misi Baru Tersedia!
              </span>

              <h1 className="text-[28px] sm:text-[36px] md:text-[48px] leading-tight tracking-tight font-extrabold font-[family-name:var(--font-outfit)] text-[#191c1e]">
                Mulai <span className="text-[#006591]">Petualangan</span> Mikroplastikmu!
              </h1>

              <p className="text-[14px] md:text-[17px] leading-relaxed text-[#3e4850] max-w-lg">
                Selami dunia yang tak terlihat di sekitarmu. Temukan bagaimana partikel kecil mengubah ekosistem kita melalui teknologi Augmented Reality.
              </p>

              {/* Logged-in student badge */}
              {currentUser?.role === 'student' && (
                <div className="bg-[#6bff8f]/20 border border-[#006e2f]/20 rounded-2xl px-4 py-3 flex items-center gap-3 max-w-sm">
                  <div className="w-9 h-9 rounded-full bg-[#006e2f]/10 flex items-center justify-center flex-shrink-0">
                    <span className="material-symbols-outlined text-[#006e2f] text-lg">school</span>
                  </div>
                  <div>
                    <p className="text-[#006e2f] text-sm font-bold">{currentUser.name}</p>
                    <p className="text-[#3e4850] text-xs">{currentUser.className || '-'} · E-LKPD masuk ke penilaian</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-1">
                <button onClick={handleStart}
                  className="flex-1 sm:flex-none bg-[#006591] hover:bg-[#004c6e] active:scale-[0.98] text-white font-bold px-7 py-3.5 rounded-2xl text-[15px] transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#006591]/25">
                  <span className="material-symbols-outlined text-lg">explore</span>
                  Mulai Perjalanan
                </button>
                {!currentUser && (
                  <Link href="/login"
                    className="flex-1 sm:flex-none border-2 border-[#bec8d2] hover:border-[#006591] text-[#006591] font-bold px-6 py-3.5 rounded-2xl text-[15px] transition-all flex items-center justify-center gap-2 bg-white active:scale-[0.98]">
              ...
              <div className="flex gap-5 pt-1 pb-2">
                {[
                  ...
                  <div key={s.label} className="text-center">
                    <div className="font-[family-name:var(--font-mono)] text-lg font-bold text-[#006591]">{s.val}</div>
                    <div className="text-[11px] text-[#6e7881]">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-6 relative hidden lg:flex items-center justify-center">
              <div className="absolute inset-0 bg-[#006e2f]/5 rounded-full scale-110 blur-3xl" />
              <img
                alt="Petualangan AR"
                className="w-full h-full object-contain z-10 rounded-3xl"
                style={{ animation: 'float 4s ease-in-out infinite', maxHeight: 480 }}
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuDu8MT8Kl3twc8c-SpOs1J12T3-QANAeqhSe5B6kwiPjP4hezUKiqDCjJQMbex8izzEG7s-hjtt9Z6XYOs-94zEZe7RH7eX5kWtDKor7xYqjLfLhCavIQ836xRPGyGVlN30bNgwmW1vN98eDA4alT65brZ7LBjBiLNRJPhnke1G1FqypNBrtZXV2N8sqVTZTnZwzJ1o1WRLAg8_x8jMlUz1RAK2KN7nAVm5TiqtQ2xXx3NqzTf66oYxSW_v_ktuWUfyf9Taz3_JTA"
              />
              <div className="absolute top-8 right-4 bg-white/90 backdrop-blur px-4 py-3 rounded-2xl shadow-xl z-20"
                style={{ animation: 'bounce 3s infinite' }}>
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-[#785a00] text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>stars</span>
                  <div>
                    <p className="text-[11px] text-[#3e4850]">Lencana Baru</p>
                    <p className="text-[13px] font-semibold">Penjaga Sungai</p>
                  </div>
                </div>
              </div>
            </div>
